...
links:
  - type: code
    url: https://github.com/benjaminsampica/bensampica.com/tree/main/src/content/blog/aspiretunit
featured: false
draft: false
toc: true
---
...
[There is a part one using TestContainers and XUnit](/blog/testcontainers), which also explains _why_ containers can improve many aspects of local development and testing.
...
![The Aspire dashboard with a sql server container resource.](images/sql-dashboard.png)
...
Just for this post, we're going to use the dbcontext's `EnsureCreatedAsync()` to build the database but in real code you'd use `MigrateAsync()`. Regardless of strategy, we only want to do this in either our test or development environments because in a production environment [you are probably deploying migrations in the pipeline](/blog/azsqlbicepefcore/). Your `Program.cs` file will look like the following after registering the `DbContext` to the service provider and having it create the database on startup. Make sure and review all the comments if you are building this yourself.
...
> [!NOTE]
> Want to just see the code? [Click here!](https://github.com/benjaminsampica/bensampica.com/tree/main/src/content/blog/aspiretunit)
...
As a final reminder you can view the code [here](https://github.com/benjaminsampica/bensampica.com/tree/main/src/content/blog/aspiretunit). That's it - happy coding!
